(function () {
    ...
    var sendToXDM = function (request, response, file, video) {
        ...
        browser.cookies.getAll({ url: response.url}, function (cookies) {
            log("Cookies: " + cookies);
    ...
    var updateBrowserAction = function () {
        ...
        if (videoList && videoList.length > 0) {
            browser.browserAction.setBadgeText({ text: videoList.length + "" });
        } else {
    ...
    var setBrowserActionPopUp = function (pop) {
        ...
        browser.browserAction.setPopup({ popup: pop });
        lastPopup = pop;
    };
    ...
    var initSelf = function () {
        ...
        browser.webRequest.onSendHeaders.addListener
            (
                ...
                { urls: ["http://*/*", "https://*/*"] },
                ["requestHeaders", "extraHeaders"] // on browser "extraHeaders" also needs to be added?
            );
                    ...
                    if (requests) {
                        if (requests.length == 1) {
                            if (!(response.url + "").startsWith(xdmHost)) {
                                log("processing request " + response.url);
